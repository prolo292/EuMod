...
/** Formatting helpers: tier ordering, mode labels, colors and Text builders. */
public final class TierFormat {
    ...
    /** Small colored tag appended to a name, e.g. " [HT1 Sword]". Null if nothing to show. */
    public static MutableText buildTag(PlayerTiers p, TierConfig cfg) {
        ...
        String inner = cfg.showModeName ? (code + " " + modeLabel(mode)) : code;
        Style gray = Style.EMPTY.withColor(TextColor.fromRgb(0x808080));

        MutableText tag = Text.literal(" [").setStyle(gray);
        tag.append(Text.literal(inner).setStyle(Style.EMPTY.withColor(TextColor.fromRgb(tierColor(code)))));
